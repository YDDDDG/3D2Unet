...
def process(para):
    ...
    metrics = getattr(module, metrics_name)(centralize=False, normalize=True).cuda()
...
def train(train_loader, model, criterion, metrics, opt, epoch, para, logger):
    ...
    for iter_samples in train_loader:
        ...
        # save_image([(i[0]*255)**2.5 / 255], 'input.png')
        # pred_trans=torch.from_numpy(cv2.cvtColor(pred[0].permute(1,2,0).detach().cpu().numpy(), cv2.COLOR_BGR2RGB).transpose(2,0,1)).float()
        
        # gt_trans=torch.from_numpy(cv2.cvtColor(gt[0].permute(1,2,0).detach().cpu().numpy(), cv2.COLOR_BGR2RGB).transpose(2,0,1)).float()
        # save_image([pred_trans,gt_trans], 'pred_and_gt/pred_gt_'+'.png')
        # cv2.imwrite("pred_and_gt/gt.png",gt[0].detach().cpu().numpy().transpose(1,2,0)*255)
        ...
        # logger.writer.add_images('input'+str(measure),i)
        # logger.writer.add_images('pred_gt'+str(measure),pred)
        # logger.writer.add_images('gt'+str(measure),gt)
